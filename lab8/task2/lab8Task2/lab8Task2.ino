...
{
  int buttonState = ((PINC&(1<<PC0))>>PC0);
  return buttonState == 0 ? true : false;
}
bool stateButton_pinA1()
{
  int buttonState = ((PINC&(1<<PC1))>>PC1);
  return buttonState == 0 ? true : false;
}
...
void myF(int i, int &num, bool state)
{
  ...
  if (stateButton_pinA0()&& !(stateButton_pinA1())) PORTD = helloSegments1[index];
  else if(stateButton_pinA1() && !stateButton_pinA0()) PORTD = helloSegments2[index];
  else if(stateButton_pinA1() && stateButton_pinA0()) PORTD = helloSegments3[index];
  else PORTD = helloSegments[index];
  delay(1);
  num = (num + 1) % 4;
}
